...
pub async fn get_data(
    ...
    // Check if this is an Oracle connector and we have both refresh_sql and filters.
    // Oracle connector doesn't support datetime filter pushdown, so we need to merge
    // filters into the SQL query instead of applying them as separate filter operations.
    let is_oracle = table_provider.as_any().downcast_ref::<OracleTableProvider>().is_some();
    let should_merge_filters_into_sql = is_oracle && sql.is_some() && !filters.is_empty();
    ...
    let mut df = match sql {
        ...
        Some(mut sql_str) => {
            // For Oracle connector with refresh_sql and filters, merge filters into SQL
            if should_merge_filters_into_sql {
                // Parse the SQL and add WHERE clause with filters
                match merge_filters_into_sql(&sql_str, &filters, ctx).await {
                    Ok(merged_sql) => {
                        tracing::debug!(
                            "Merged filters into SQL for Oracle connector: {}",
                            merged_sql
                        );
                        sql_str = merged_sql;
                    }
                    Err(e) => {
                        tracing::warn!(
                            "Failed to merge filters into SQL for Oracle connector, falling back to filter operations: {}",
                            e
                        );
                        // Continue with original SQL, filters will be applied separately
                    }
                }
            }
    ...
    // Only apply filters as separate operations if we didn't merge them into SQL
    if !should_merge_filters_into_sql {
...
/// Merges filters into SQL query by adding WHERE clause.
/// This is needed for Oracle connector which doesn't support datetime filter pushdown.
/// 
/// The approach: Build a logical plan with filters, then convert it back to SQL.
/// This ensures filters are properly merged into the SQL query.
fn merge_filters_into_sql(
    sql: &str,
    filters: &[Expr],
    ctx: &mut SessionContext,
) -> Result<String, DataFusionError> {
    // Parse the original SQL into a logical plan
    let mut plan = ctx
        .state()
    ...
    // Convert the plan back to SQL
    let unparser = Unparser::default();
    let sql_string = unparser
        .plan_to_sql(&plan)
        .map_err(|e| DataFusionError::Internal(format!("Failed to convert plan to SQL: {}", e)))?;
